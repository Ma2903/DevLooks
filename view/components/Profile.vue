    ...
    <div class="bg-gray-900 p-16 rounded-3xl shadow-2xl w-full max-w-4xl">
      <div class="text-center mb-10 relative">
        ...
        <p class="text-gray-400 mt-3 text-lg flex items-center justify-center">
         ...
         <p v-if="userData && userData.avatarPasses > 0" class="text-yellow-400 mt-3 text-lg">
            <i class="fas fa-ticket-alt"></i> Passes de Avatar: {{ userData.avatarPasses }}
        </p>
      </div>
      ...
      <div v-if="userData.savedAvatars && userData.savedAvatars.length > 0" class="mt-8">
        <h2 class="text-xl font-bold text-center text-[#04d1b0] mb-4">Meus Avatares</h2>
        <div class="flex justify-center flex-wrap gap-4 bg-gray-800/50 p-4 rounded-lg">
          <img
            v-for="(avatar, index) in userData.savedAvatars"
            :key="index"
            :src="avatar"
            alt="Avatar Salvo"
            class="w-20 h-20 rounded-full border-2 cursor-pointer transition-transform transform hover:scale-110"
            :class="userData.avatarUrl === avatar ? 'border-[#04d1b0]' : 'border-gray-600'"
...
export default {
  ...
  async mounted() {
    // ... (o seu mounted hook continua igual)
     if (!this.token) {
  ...
  methods: {
    // ... (os seus métodos confirmLogout, logout, confirmDelete, deleteAccount continuam iguais)
     confirmLogout() {
      Swal.fire({
        title: 'Você tem certeza?', text: "Você será desconectado da sua conta.", icon: 'warning', showCancelButton: true,
        confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Sim, sair!', cancelButtonText: 'Cancelar',
    ...
    confirmDelete() {
      Swal.fire({
        title: 'Deletar sua conta?', text: "Esta ação é irreversível! Para confirmar, digite 'DELETAR' abaixo:", icon: 'error',
        input: 'text', inputPlaceholder: 'DELETAR', showCancelButton: true, confirmButtonText: 'Deletar', cancelButtonText: 'Cancelar',
        confirmButtonColor: '#d33', background: "#1F2937", color: "#E5E7EB",
        ...
        if (result.isConfirmed) this.deleteAccount();
      });
    },
    async deleteAccount() {
      if (!this.userData || !this.userData._id) {
        Swal.fire({ title: 'Erro de Sessão', text: 'Não foi possível encontrar os dados do seu usuário. Por favor, faça o login novamente.', icon: 'warning', background: "#1F2937", color: "#E5E7EB" });
        this.logout();
        return;
      }
      try {
        await axios.delete(`/api/users/${this.userData._id}`, { headers: { Authorization: `Bearer ${this.token}` } });
        Swal.fire({ title: 'Conta Deletada!', text: 'Sua conta foi removida com sucesso.', icon: 'success', background: "#1F2937", color: "#E5E7EB" });
        this.logout();
      } catch (err) {
        Swal.fire({ title: 'Erro!', text: 'Não foi possível deletar a sua conta.', icon: 'error', background: "#1F2937", color: "#E5E7EB" });
      }
    },
    // NOVO MÉTODO PARA TROCAR A FOTO DE PERFIL
    async changeProfilePicture(newAvatarUrl) {
      if (this.userData.avatarUrl === newAvatarUrl) return; // Não faz nada se já for a foto atual

      try {
        // A rota de `updateUser` pode ser usada para alterar campos específicos
        const response = await axios.put(`/api/users/${this.userData._id}`, 
          { avatarUrl: newAvatarUrl }, // Enviamos apenas o campo que queremos mudar
          { headers: { Authorization: `Bearer ${this.token}` } }
        );
